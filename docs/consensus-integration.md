...
- [`examples/openraft-standalone`](https://github.com/prisma-risk/tsoracle/tree/main/examples/openraft-standalone) uses the bundled `StandaloneHost`, which owns its own raft cluster + `HighWaterStateMachine`. Pick this when TSO gets its own cluster. The example shows the minimum bring-up (rocksdb log store, tonic peer transport) plus `NodeId -> tsoracle-addr` resolution wired through `OpenraftDriver::with_peers` (a list of `TsoPeer { node_id, endpoint }`) for `LeaderHint` follower-redirect.
