...
## The `now_ms` contract: advisory only

Every `Allocator` method that needs physical time accepts it as a `now_ms: u64` parameter. This value is *advisory*: it tells the allocator how to advance physical time toward the wall clock. The allocator's monotonicity does not depend on the value's correctness — a `now_ms` that jumps backward cannot cause regression because the persisted high-water always wins. A `now_ms` pinned far in the past stalls new windows until later samples catch up past the persisted bound. The choice of clock source (`SystemTime`, NTP-disciplined, HLC-extended, externally driven) is therefore the embedder's concern, not the algorithm's, and does not require any changes to the allocator's correctness reasoning.
