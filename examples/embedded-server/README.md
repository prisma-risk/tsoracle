...
- **You want to share a tonic listener with other services.** Use [`Server::into_router`](https://docs.rs/tsoracle-server/latest/tsoracle_server/struct.Server.html#method.into_router) instead of `serve_with_shutdown`; it returns a `Result` wrapping a tonic `Routes` value plus a `JoinHandle<Result<(), ServerError>>` for the leader-watch task (the outer `Result` surfaces a `ServerError::ReflectionInit` if the embedded descriptor set fails to decode under the `reflection` feature). Keep and observe the handle so watch failures are visible to your embedding process.
