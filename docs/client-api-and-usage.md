...
**`batch_flush_interval`** controls coalescing latency: the background driver buffers concurrent `get_ts` / `get_ts_batch` calls and flushes them as one outgoing `GetTs` once this interval has elapsed since the first buffered call (default: 1 ms). Lowering it reduces latency for individual calls at the cost of more outgoing RPCs; raising it amortizes more RPC cost at the cost of latency. For workloads that already batch explicitly, the value is largely irrelevant — concurrent batches still coalesce, but the per-call gain is smaller.
