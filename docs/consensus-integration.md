...
This differs from dense sequences: `advance_dense` is a non-idempotent fetch-add and must commit exactly once, while `persist_leases` is idempotent full-state replacement. A grant that supersedes an older lease persists both records in one full-set write, making supersession atomic. The file driver is the single-node reference implementation: it stores a versioned, checksummed `leases` file with tmpfile fsync, rename, and directory fsync.

Full-set replacement changes what fencing has to do in a replicated driver. `persist_high_water` can absorb a stale proposer at apply time with `max(prev, at_least)`, because a stale advance never lowers the high-water. A stale lease write has no such merge: a set projected before a leadership change can lack leases granted after it, and applying it would drop them from durable state. A replicated driver must therefore apply a lease write only when it commits under the `epoch` it was projected in, and return `ConsensusError::Fenced` otherwise. The server guards its side too: it projects the set under the same epoch check that guards its in-memory commit.

The openraft driver replicates leases as a `SetLeases` log command carrying the full set and the raft term named by `epoch`. The state machine applies it only when the entry's own log id carries that term, and the set is part of the state-machine snapshot. Leases are a format feature there: they need write version 7, and `persist_leases` returns `ConsensusError::LeasesNotActivated` until the cluster activates it (see [Format Migration](format-migration-upgrade.md)). The paxos driver does not persist leases.
