...
The task exits cleanly when the upstream `leadership_events` stream ends (driver shutdown). If the watch task returns an error, it poisons serving state before returning and `Server::serve_with_shutdown` shuts tonic down gracefully while surfacing the error. Embedders using `Server::into_router` should keep and observe the returned `JoinHandle<Result<(), ServerError>>`.
