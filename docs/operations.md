...
- `tsoracle.window.extensions.ignored.epoch_mismatch.total` — a persisted window extension was dropped because it targeted a superseded epoch (fenced out); sustained non-zero rates indicate epoch churn (counter)
- `tsoracle.window.extensions.ignored.not_advanced.total` — a persisted window extension did not exceed the current durable bound, so the monotonic bound rejected it; non-zero rates indicate persist reordering (counter)
...
- `tsoracle.leadership_stream.rejected.total` — a `leadership_events()` subscription was refused because a live leadership stream was already held; the driver fails closed and returns an empty stream (counter). At most one server may consume a driver's leadership stream at a time, so a single transient bump is a benign concurrent-restart race. A *sustained* non-zero rate means the single-active lease is wedged — a server was torn down without dropping its stream (a leaked `WatchGuard` or detached watch task), leaving the node permanently `NotServing`; recover by restarting the process. The companion `tracing::error!` carries the `rejected_generation` and `active_generation` ids identifying the wedged holder.
