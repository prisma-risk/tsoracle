...
| `tsoracle-core` | The window allocator state machine, `Clock` trait, `Epoch` type, `Timestamp` packing, and the dense-sequence keying/validation types (`SeqKey`, `SeqGrant`). Pure sync. No `tokio`, no I/O. |
| `tsoracle-consensus` | The `ConsensusDriver` trait (timestamp persistence plus the optional `advance_dense`/`load_dense_seq` methods) and shared types. Where async first appears, but only as a method signature — no runtime is assumed. |
